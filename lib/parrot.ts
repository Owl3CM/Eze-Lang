 ...
 */
function loadUserConfig(): ParrotConfig {
  const userConfig: ParrotConfig = require("./parrot.config.json");
  return userConfig;
  //   try {
...
function generateTypes(blueprint: Blueprint): string {
  ...
  for (const group in blueprint) {
    for (const key in blueprint[group]) {
      const node = blueprint[group][key] as BlueprintNode & Record<string, any>;
      const { desc, placeholders, parrotHolders, value, conditions, variants } = node;

      // Convert placeholders from CSV to arrays if needed
      const placeholderArray = placeholders?.split(",").map((s) => s.trim()) ?? [];

      const parrotHoldersArray = parrotHolders?.split(",").map((s) => s.trim()) ?? [];
      ...
      // Determine if node is dynamic or static
      let isDynamic = false;
      let typeContent = "";

      if (placeholderArray.length > 0) {
        isDynamic = true;
        const paramsArr = placeholderArray.map((phKey, i) => {
          if (i === 0 && variants) {
            const variantsUnion = Object.keys(variants).join('" | "');
            return `${phKey}: "${variantsUnion}"${value ? ' | ""' : ""}`;
          }
          return `${phKey}: string | number`;
        });

        if (parrotHoldersArray.length > 0) {
          paramsArr.push(...parrotHoldersArray.map((phKey) => `${phKey}: ParrotKey`));
        }
        const params = `{ ${paramsArr.join("; ")} }`;
        typeContent = `  ${key}: (params: ${params}) => string;\n`;
      } else if (parrotHoldersArray.length > 0) {
        isDynamic = true;
        const params = `{ ${parrotHoldersArray.map((phKey) => `${phKey}: ParrotKey`).join("; ")} }`;
        typeContent = `  ${key}: (params: ${params}) => string;\n`;
  ...
  // Compose final interface definitions
...
async function main(): Promise<void> {
  try {
    ...
    const yamlDir = path.join(config.ymlDir);
    ...
    try {
      execSync(`npx prettier --write ${config.outputDir}`, { stdio: "inherit" });
    } catch (err: any) {
